...
class UIController {
     ...
     */
    async updateDashboard(trades, summary) {
        ...
        // Prefetch which days have screenshots (for calendar journal indicators)
        await this._prefetchScreenshotDays();
     ...
     */
    async openJournalModal(dayKey, trades, focusTradeId) {
        ...
        // Populate content (async — loads screenshots from IndexedDB)
        await this._populateJournalTab(dayKey, trades, focusTradeId);
    ...
    async closeJournalModal() {
        ...
        // Refresh screenshot day cache, then re-render calendar to update journal indicators
        await this._prefetchScreenshotDays();
        this.renderCalendar();
    }
    ...
    // ===== Populate Journal Tab (async — loads images from IndexedDB) =====

    async _populateJournalTab(dayKey, trades, focusTradeId) {
        ...
        dayScreenshots.forEach(s => {
            if (!screenshotsByTrade[s.tradeId]) screenshotsByTrade[s.tradeId] = [];
            screenshotsByTrade[s.tradeId].push(s);
        });
        ...
        trades.forEach((trade, i) => {
            ...
            // Get screenshots from IndexedDB batch
            const tradeScreenshots = screenshotsByTrade[tradeId] || [];
                ...
                <div class="journal-trade-entry" data-trade-id="${tradeId}">
                    ...
                    <div class="journal-trade-entry-body${collapsed}">
                        ...
                        <div class="journal-screenshot-grid" data-trade-id="${tradeId}">
                            ${tradeScreenshots.map((s, si) => `
                                <div class="journal-screenshot-thumb" data-img-id="${s.id}" data-index="${si}">
                                    <img src="${s.dataUrl}" alt="Screenshot ${si + 1}" loading="lazy">
    ...
    _bindJournalEvents(dayKey, trades) {
        ...
        document.querySelectorAll('.screenshot-remove').forEach(btn => {
            btn.onclick = async (e) => {
                e.stopPropagation();
                await this._handleScreenshotRemove(btn.dataset.imgId, btn.closest('.journal-screenshot-thumb'));
            };
        });
        ...
        // Auto-save on modal close (notes only — screenshots save immediately to IDB)
        this._journalAutoSave = () => this._saveCurrentJournal(dayKey, trades);
    }

    // ===== Save Current Journal State (notes only — screenshots are in IndexedDB) =====
    ...
    _saveCurrentJournal(dayKey, trades) {
        ...
        trades.forEach((trade, i) => {
            ...
            journal.trades[tradeId] = {
                ...
                // screenshots are in IndexedDB, not here anymore
            };
        });
    ...
    // ===== Screenshot Remove — deletes from IndexedDB =====

    async _handleScreenshotRemove(imgId, thumbEl) {
        if (imgId) {
            try { await ImageStore.remove(imgId); } catch (e) { console.warn('Failed to remove from IDB:', e); }
        }
    ...
    _addScreenshotToGrid(tradeId, dataUrl, imgId) {
        ...
        thumb.dataset.imgId = imgId || '';
        thumb.dataset.index = thumbCount;
        thumb.innerHTML = `
            <img src="${dataUrl}" alt="Screenshot" loading="lazy">
            <button class="screenshot-remove" data-img-id="${imgId || ''}" data-trade-id="${tradeId}" data-index="${thumbCount}" title="Remove">&times;</button>
        `;
        ...
        thumb.querySelector('.screenshot-remove').onclick = async (e) => {
            e.stopPropagation();
            await this._handleScreenshotRemove(imgId, thumb);
        };
    }
    ...
    _readImageFile(file, callback) {
        ...
        reader.onload = (e) => {
            ...
            img.onload = () => {
                ...
                // Higher quality now that we have unlimited storage
    ...
    // Checks localStorage for notes synchronously + uses a prefetched cache for IndexedDB screenshots
    ...
    _dayHasJournal(dayKey) {
        ...
        // Also check the screenshot cache (prefilled by _prefetchScreenshotDays)
        if (this._daysWithScreenshots && this._daysWithScreenshots.has(dayKey)) return true;
        return false;
    }

    // Prefetch which days have screenshots stored in IndexedDB (called once before calendar render)
    async _prefetchScreenshotDays() {
        try {
            this._daysWithScreenshots = await ImageStore.getDaysWithScreenshots();
        } catch (e) {
            console.warn('Failed to prefetch screenshot days:', e);
            this._daysWithScreenshots = new Set();
        }
    }
